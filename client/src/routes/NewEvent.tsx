...
import { StayingEventDefault, MovingEventLabelDefaults, StayingEventLabelDefaults } from '../utils/defaults';
        ...
        >
            <>
                ...
                <div className="navigate-event">
                    <div className="unique-event-code">{eventCode.toUpperCase()}</div>
                    <input
                        type="text"
...
const NewEvent = () => {
    ...
    const [eventInfo, setEventInfo] = useState<Event>(StayingEventDefault);
    ...
    // for logging
    useEffect(() => {
        console.log('The event type is ', eventInfo.type)
    }, [eventInfo])
        ...
        if (mapFile) { // Upload this to the event maps bucket
            ...
            uploadEventMap(mapFile).then(({ data: image, error: mapError }) => {
                ...
                createNewEvent({
                    ...
                    project: projectId,
                    type: eventInfo.type,
                    label_mapping0: eventInfo.label_mapping0 || MovingEventLabelDefaults.label_mapping0,
                    label_mapping1: eventInfo.label_mapping1 || MovingEventLabelDefaults.label_mapping1,
                    label_mapping2: eventInfo.label_mapping2 || MovingEventLabelDefaults.label_mapping2,
                    label_mapping3: eventInfo.label_mapping3 || MovingEventLabelDefaults.label_mapping3,
                    label_mapping4: eventInfo.label_mapping4 || MovingEventLabelDefaults.label_mapping4,
                                ...
                                <div className="input-box">
                                    ...
                                    <select
                                        ...
                                        onChange={(event) => {
                                            ...
                                            // todo: change to != 
                                            if (eventType == 'STAY' && eventInfo.type == 'MOVE') {
                                                setEventInfo(
                                                    (prevState) => ({
                                                        ...prevState,
                                                        type: eventType,
                                                        ...StayingEventLabelDefaults
                                                    })
                                                )
                                            } else if (eventType == 'MOVE' && eventInfo.type == 'STAY') {
                                                setEventInfo(
                                                    (prevState) => ({
                                                        ...prevState,
                                                        type: eventType,
                                                        ...MovingEventLabelDefaults
                                                    })
                                                )
                                            } else {
                                                // nothing to do since we're not updating anything
                                            }
                                        }}
                                    >
                                        <option value="STAY">Activity Mapping</option>
                                        <option value="MOVE">Moving Activity</option>
                                    </select>
                                </div>
                                <div className='help-text'>
                                    {
                                        eventInfo.type == 'STAY' ?
                                            <span>Record <b>still movements</b> such as sitting or standing as coordinates
                                                on your event map.</span> :
                                            <span>Record <b>movement</b> such as walking or driving with a line marker
                                                on your event map.</span>
                                    }
                                </div>
                            </div>
